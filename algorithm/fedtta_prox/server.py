...
from algorithm.fedtta_prox.client import CLIENT
        ...
        for i in tqdm(range(self.config.num_rounds)):
            ...
            if i == 0 or (i + 1) % self.config.eval_interval == 0:
                if self.config.save_models and not self.config.direct_test:
                    ...
                    torch.save(self.model, f'./{self.config.path}/fedtta_prox/{i}-model.pt')
                    torch.save(self.loss_nn, f'./{self.config.path}/fedtta_prox/{i}-lossnn.pt')

                if self.config.direct_test:
                    self.model = torch.load(f'./{self.config.path}/fedtta_prox/{i}-model.pt')
                    self.model_params = self.model.state_dict()
                    self.loss_nn = torch.load(f'./{self.config.path}/fedtta_prox/{i}-lossnn.pt')
                    self.loss_nn_params = self.loss_nn.state_dict()
