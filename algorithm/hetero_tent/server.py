...
from algorithm.hetero_tent.client import CLIENT
...
class SOFT_DATASET(Dataset):
    def __init__(self, X, Y, personalzied_Y=None, transform=None):
        self.X = X
        self.Y = Y
...
class SERVER:
    ...
    def ensemble_knowledge(self, clients, round_th):
        ...
        for c in clients:
            ...
            return_model = c.finetune(model=c.model, round_th=round_th, data_loader=public_dataloader, return_finetuned_model=True)
        ...
        public_x, public_y, public_personalized_avg_logits, transform = public_dataloader.dataset.X, public_dataloader.dataset.Y, personalized_avg_logits, public_dataloader.dataset.transform
        new_public_dataset = SOFT_DATASET(X=public_x, Y=public_y, personalzied_Y=public_personalized_avg_logits, transform=transform)
        ...
        return personalized_ensemble_acc
    def federate(self):
        ...
        for i in tqdm(range(self.config.num_rounds)):
            ...
            for k in range(len(self.selected_clients)):
                ...
                if i == 0:
                    train_samples_num, c_model_params, loss = c.train(round_th=i, digest=False, revisit=True)
                else:
                    train_samples_num, c_model_params, loss = c.train(round_th=i, digest=True, revisit=True)
            personalized_ensemble_public_acc = self.ensemble_knowledge(clients=self.selected_clients, round_th=i)
