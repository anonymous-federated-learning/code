...
class CLIENT:
    ...
    def train(self, round_th):
        ...
        prox_model = deepcopy(model)
        model.to(self.device), loss_nn.to(self.device)
        prox_model.to(self.device)
        ...
        for it in range(self.config.local_iters):
            ...
            with higher.innerloop_ctx(model, inner_opt, copy_initial_weights=False) as (fnet, diffopt):
                ...
                for _ in range(self.config.inner_iters):
                    ...
                    prox_logits = prox_model(spt_x)
                    spt_loss = loss_nn(spt_logits)
                    diffopt.step(spt_loss)
                eval_logits = fnet(spt_x)
                # latter -> teacher
                prox_kl_loss = self.loss_kl(F.log_softmax(spt_logits, dim=1), F.softmax(prox_logits.detach(), dim=1))
                empirical_loss = self.loss_ce(eval_logits, spt_y)
                loss = empirical_loss + self.config.tta_prox * prox_kl_loss
                meta_opt.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), 10)
                torch.nn.utils.clip_grad_norm_(loss_nn.parameters(), 10)
                meta_opt.step()
                mean_loss.append(empirical_loss.item())
    ...
    def finetune(self, model, loss_nn, round_th, data_loader, patience=5):
        ...
        for it in range(finetune_iters):
            ...
            for step, (spt_x, spt_y) in enumerate(data_loader):
                ...
                copy_logits = spt_logits.clone().detach_()
                probability = F.softmax(copy_logits, dim=1)
                b = p * torch.log2(probability)
                # b = p * torch.log(probability)
                # b = F.softmax(copy_logits, dim=1) * F.log_softmax(copy_logits, dim=1)
                e = -1.0 * b.sum() / b.shape[0]
                entropy += e
                # -------------------
                spt_loss += loss_nn(spt_logits)
                num_batches += 1
            ...
            if entropy < best_entropy:
                ...
                best_entropy_iter = it #
            else:
                p += 1

        return samples, acc_list[best_entropy_iter], loss_list[best_entropy_iter]
