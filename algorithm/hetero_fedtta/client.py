...
class CLIENT:
    def __init__(self, user_id, train_loader, validation_loader, test_loader, config):
        ...
        self.loss_nn = get_hetero_loss_nn(num_classes=config.num_classes, model_type=model_type)
    ...
    def reset_models(self):
        ...
        self.loss_nn = get_hetero_loss_nn(num_classes=self.config.num_classes, model_type=model_type)
        ...
        if digest: #
            ...
            for it in range(distillation_epochs):
                for _, all_ in enumerate(self.public_dataloader):
                    x, y, base_avg_logits, personalized_avg_logits = all_[0], all_[1], all_[2], all_[3]
                    x, base_avg_logits, personalized_avg_logits = x.to(self.device), base_avg_logits.to(self.device), personalized_avg_logits.to(self.device)
                    with higher.innerloop_ctx(model, inner_opt, copy_initial_weights=False) as (fnet, diffopt):
                        ...
                        eval_logits = fnet(x)
                        distillation_loss = self.loss_kl(F.log_softmax(eval_logits, dim=1), F.softmax(personalized_avg_logits.detach(), dim=1)) \
                                            + self.config.base_distillation * self.loss_kl(F.log_softmax(spt_logits, dim=1), F.softmax(base_avg_logits.detach(), dim=1))
                        meta_opt.zero_grad()
        ...
        if revisit:
            ...
            for it in range(self.config.local_iters):
                ...
                with higher.innerloop_ctx(model, inner_opt, copy_initial_weights=False) as (fnet, diffopt):
                    ...
                    eval_loss = self.loss_ce(eval_logits, spt_y)
                    meta_opt.zero_grad()
                    eval_loss.backward()
                    torch.nn.utils.clip_grad_norm_(model.parameters(), 10)
                    torch.nn.utils.clip_grad_norm_(loss_nn.parameters(), 10)
                    meta_opt.step()
                    mean_loss.append(eval_loss.item())
            model, loss_nn = self.get_nns()
            ...
            if np.isnan(sum(mean_loss) / len(mean_loss)):
                ...
                # exit(0)
            return self.train_samples_num, model, loss_nn, sum(mean_loss) / len(mean_loss)

    def finetune(self, model, loss_nn, round_th, data_loader, return_finetuned_model=False):
        ...
        for it in range(self.config.finetune_iters):
            ...
            num_batches = 0
            for step, all_ in enumerate(data_loader):
                spt_x, spt_y = all_[0].to(self.device), all_[1].to(self.device)
    ...
    def get_nns(self):
        return deepcopy(self.model), deepcopy(self.loss_nn)
